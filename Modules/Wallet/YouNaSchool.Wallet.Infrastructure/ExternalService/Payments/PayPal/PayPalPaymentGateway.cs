...
namespace YouNaSchool.Wallet.Infrastructure.ExternalService.Payments.PayPal
{
    public sealed class PayPalPaymentGateway : IPaymentService
    {
        ...
        public PayPalPaymentGateway(HttpClient http,PayPalAuthService auth, IOptions<PayPalSettings> options)
        {
            _http = http;
            _auth = auth;
                ...
                JsonSerializer.Serialize(new
                {
                    ...
                    purchase_units = new[]
                    {
                        new
                        {
                            ...
                            rechargeId = command.RechargeId.ToString() // Custom field for internal tracking
                        }
                    },
            ...
            IReadOnlyDictionary<string, string> headers)
        {
            ...
            return new PaymentWebhookResult
            {
                ...
                Amount = resource.GetProperty("purchase_units")[0].GetProperty("amount").GetProperty("value").GetDecimal(),
                RechargeId = Guid.Parse(resource.GetProperty("purchase_units")[0].GetProperty("rechargeId").GetString()!)
            };
        }
